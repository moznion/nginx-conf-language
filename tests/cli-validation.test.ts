...
describe('CLI Validation Features', () => {
  ...
  it.skipIf(!dockerAvailable)('should validate NCL file with --validate option', () => {
    const nclContent = `
    ...
    const result = execSync(`npx tsx src/cli/index.ts "${nclPath}" --validate`, {
      encoding: 'utf-8',
      cwd: process.cwd()
    });
  ...
  it.skipIf(!dockerAvailable)('should validate NCL file with --validate-only option', () => {
    const nclContent = `
      worker_processes auto;
    ...
    const result = execSync(`npx tsx src/cli/index.ts "${nclPath}" --validate-only`, {
      encoding: 'utf-8',
      cwd: process.cwd()
    });
  ...
  it.skipIf(!dockerAvailable)('should work with location regex patterns', () => {
    const nclContent = `
      worker_processes auto;
