...
# Another comment`;
  ...
  it('should handle environment variables in comprehensive configuration', () => {
    ...
    const input = readFileSync(join(__dirname, '../examples/environment-demo.ncl'), 'utf-8');
    const ast = parse(input);
    const output = generate(ast, { expandInline: true });
  ...
  it('should error on missing required environment variable', () => {
    ...
    expect(() => parse(input))
      .toThrow('Environment variable REQUIRED_VAR is not set and no default value provided');
  });
});
