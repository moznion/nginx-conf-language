...
export class Parser {
	...
	private parseVariableAssignment(): VariableAssignmentNode {
		...
		if (
			!this.consume(TokenType.Semicolon, "Expected ; after variable assignment")
		) {
			throw this.error("Expected ; after variable assignment");
		}
	...
	private parseLocation(): LocationNode {
		...
		if (this.check(TokenType.In)) {
			...
			if (
				!this.consume(TokenType.LeftBracket, "Expected [ after location in")
			) {
			...
			while (!this.check(TokenType.RightBracket) && !this.isAtEnd()) {
				if (!this.check(TokenType.String) && !this.check(TokenType.EnvVar)) {
					throw this.error(
						"Expected string or environment variable in location path list",
					);
				}
			...
			if (
				!this.consume(TokenType.RightBracket, "Expected ] after location paths")
			) {
	...
	private parseLocationModifier(value: string): LocationModifier {
		switch (value) {
			case "=":
				return LocationModifier.Exact;
			case "~":
				return LocationModifier.Regex;
			case "~*":
				return LocationModifier.RegexCaseInsensitive;
			case "^~":
				return LocationModifier.Prefix;
			default:
				return LocationModifier.None;
		}
	}
	...
	private parseDirective(): DirectiveNode {
		...
		while (!this.check(TokenType.Semicolon) && !this.isAtEnd()) {
			...
			if (
				currentToken.line > directiveLine &&
				this.check(TokenType.Identifier)
			) {
				break;
			}
	...
	private parseEnvironmentVariable(): EnvironmentVariableNode {
		...
		if (
			!this.consume(
				...
				"Expected ) after environment variable",
			)
		) {
