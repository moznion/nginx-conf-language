...
export class NginxValidator {
  ...
  private async isDockerAvailable(): Promise<boolean> {
    try {
      execSync('docker --version', { stdio: 'pipe', timeout: this.options.timeout });
   ...
   */
  private async validateWithDocker(confPath: string): Promise<ValidationResult> {
    try {
   ...
   */
  private async validateWithLocalNginx(confPath: string): Promise<ValidationResult> {
    try {
      ...
      if (error.code === 'ENOENT' || 
        ...
        return {
          ...
          errors: [`nginx is not installed or not in PATH: ${this.options.nginxCommand}`],
          warnings: ['Consider installing nginx or using Docker validation (--use-docker)']
        };
      }
